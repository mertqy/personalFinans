...
class GoalsTab extends ConsumerWidget {
        ...
        children: [
          ...
          if (activeGoals.isNotEmpty) ...[
            const Padding(
              ...
              child: Text('Aktif Hedefler', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            ),
          ...
          if (completedGoals.isNotEmpty) ...[
            const Padding(
              ...
              child: Text('Tamamlanan Hedefler', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.grey)),
            ),
  ...
  Widget _buildGoalCard(BuildContext context, WidgetRef ref, Goal goal) {
    ...
    final themeColor = Color(int.parse((goal.levelColor.isEmpty ? '#64B5F6' : goal.levelColor).replaceFirst('#', 'ff'), radix: 16));
  ...
  void _showDepositDialog(BuildContext context, WidgetRef ref, Goal goal) {
    ...
    final accounts = ref.read(accountProvider).where((acc) => acc.type != 'investment').toList();
  ...
  void _confirmDelete(BuildContext context, WidgetRef ref, Goal goal) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('${goal.title} hedefini silmek istediğinize emin misiniz? Biriken tutar hesaplara geri aktarılmaz.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('İptal')),
          ElevatedButton(
            onPressed: () {
