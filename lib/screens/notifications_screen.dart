...
class NotificationsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final lateTransactions = plannedTransactions
        .where((t) => t.date.isBefore(today))
        .toList();
    final upcomingTransactions =
        plannedTransactions.where((t) => !t.date.isBefore(today)).toList()
    ...
    final hasContent =
        ...
        budgetWarnings.isNotEmpty ||
        cardReminders.isNotEmpty ||
        subReminders.isNotEmpty;
          ...
          : ListView(
              ...
              children: [
                ...
                if (budgetWarnings.isNotEmpty) ...[
                  ...
                  ...budgetWarnings.map(
                    (w) => _buildBudgetWarningCard(context, w),
                  ),
                  const SizedBox(height: 24),
                ],
                ...
                if (cardReminders.isNotEmpty) ...[
                  ...
                  ...cardReminders.map(
                    (r) => _buildCardReminderCard(context, r),
                  ),
                  const SizedBox(height: 24),
                ],
                ...
                if (lateTransactions.isNotEmpty) ...[
                  _buildSectionTitle(
                    context,
                    'Gecikmiş Ödemeler/Planlar',
                    Colors.red,
                  ),
                  ...lateTransactions.map(
                    (tx) =>
                        _buildNotificationCard(context, ref, tx, isLate: true),
                  ),
                  const SizedBox(height: 24),
                ],
                ...
                if (upcomingTransactions.isNotEmpty) ...[
                  ...
                  ...upcomingTransactions.map(
                    (tx) => _buildNotificationCard(context, ref, tx),
                  ),
                ],
              ],
            ),
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (isOver ? Colors.red : Colors.orange).withValues(
                      alpha: 0.1,
                    ),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Text(
                    AppUtils.getCategoryIcon(categoryId),
            ...
            Row(
              ...
              children: [
                Text(
                  '${AppUtils.formatCurrency(spent)} harcandı',
  ...
  Widget _buildCardReminderCard(
    ...
    final title = isDue
        ? '${card.bank} - Son Ödeme Günü'
        : '${card.bank} - Hesap Kesim';
    final subtitle = daysLeft == 0
        ? (isDue ? 'Son ödeme günü bugün!' : 'Hesap kesim bugün!')
        : (isDue
              ? '$daysLeft gün sonra son ödeme'
              : '$daysLeft gün sonra hesap kesim');
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (isDue)
              Text(
                'Borç: ${AppUtils.formatCurrency(card.currentDebt)}',
  ...
  void _confirmComplete(BuildContext context, WidgetRef ref, Transaction tx) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          '"${tx.description}" işlemini bugün gerçekleşmiş olarak işaretlemek istiyor musunuz? Bakiyeniz güncellenecektir.',
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              ref
                  ...
                  .completePlannedTransaction(tx);
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('İşlem tamamlandı ve bakiye güncellendi.'),
                ),
              );
            },
