...
class _AddLoanModalState extends ConsumerState<AddLoanModal> {
  ...
  void initState() {
    ...
    if (_selectedName == null && widget.loan == null) {
      _selectedName = _loanNames.first;
    }

    final accounts = ref.read(accountProvider);
    final loanAccount = accounts
        .where((a) => a.id == widget.loan?.accountId)
        .firstOrNull;
    final displayTotal = widget.loan != null
        ? AppUtils.convertToBaseCurrency(
            widget.loan!.totalAmount,
            loanAccount?.currency ?? 'TRY',
            'TRY',
          )
        : null;
    ...
    _totalController = TextEditingController(
      text: displayTotal != null
          ? ThousandsSeparatorInputFormatter.format(displayTotal)
          : '',
    );
    _paymentController = TextEditingController(
      text: displayPayment != null
          ? ThousandsSeparatorInputFormatter.format(displayPayment)
          : '',
    );
  ...
  void _calculate() {
    ...
    if (months > 0) {
      ...
      setState(() {
        ...
        _paymentController.text = ThousandsSeparatorInputFormatter.format(
          monthly,
        );
      });
    }
  ...
  void _save() {
    if (_formKey.currentState!.validate()) {
      ...
      final paymentInTry = ThousandsSeparatorInputFormatter.parse(
        _paymentController.text,
      );
      final principalInTry = ThousandsSeparatorInputFormatter.parse(
        _totalController.text,
      );
      ...
      final payment = AppUtils.convertToBaseCurrency(
        paymentInTry,
        'TRY',
        currency,
      );
      final principal = AppUtils.convertToBaseCurrency(
        principalInTry,
        'TRY',
        currency,
      );
      final totalRepayment = AppUtils.convertToBaseCurrency(
        totalRepaymentInTry,
        'TRY',
        currency,
      );
      ...
      if (widget.loan != null) {
        ...
        final loan = Loan(
          ...
          type: _selectedName == 'Araç'
              ? 'auto'
              : (_selectedName == 'Konut' ? 'mortgage' : 'personal'),
          ...
          endDate: DateTime.now().add(
            Duration(days: (int.tryParse(_durationController.text) ?? 12) * 30),
          ),
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Text(
                    isEditing ? 'Krediyi Düzenle' : 'Yeni Kredi Ekle',
              ...
              DropdownButtonFormField<String>(
                initialValue: _loanNames.contains(_selectedName)
                    ? _selectedName
                    : null,
                items: _loanNames
                    .map(
